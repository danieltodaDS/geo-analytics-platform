...
```
O que chega:    Parquets em data/raw/ (Phase 4a) ou raw no BigQuery (Phase 4b+)
...
em Phase 4b é o `raw` do BigQuery. O que muda entre fases é apenas
...
Substituir `_sources.yml` por sources apontando para `raw` no BigQuery:
